...
@Component
public class MeetFileHandler {

    private final PhotoService photoService;

    public MeetFileHandler(PhotoService photoService) {
        this.photoService = photoService;
    }

    public List<MeetPhoto> parseFileInfo(
    	Meet meet,
    	List<MultipartFile> multipartFiles
    )throws Exception {
  	// 반환할 파일 리스트
        List<MeetPhoto> fileList = new ArrayList<>();
	...
	// 전달되어 온 파일이 존재할 경우
        if(!CollectionUtils.isEmpty(multipartFiles)) { 
            ...
            // 파일을 저장할 세부 경로 지정
            String path = "images/meet" + File.separator + current_date;
